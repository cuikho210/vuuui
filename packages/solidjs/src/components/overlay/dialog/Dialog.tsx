...
export const Dialog: DialogComponent = (props) => {
  ...
  return (
    <dialog {...props} open={open()} title="" class={className()} tabindex="-1">
      ...
      <Container class="vuuui-body">
        <div class="vuuui-title">
          <div class="vuuui-title-left">
            <div class="vuuui-button-slot">{props.action}</div>
            <span class="vuuui-title-content"> {props.title} </span>
          </div>

          <div class="vuuui-button-slot">{props.closeButton}</div>
        </div>

        <div class="vuuui-content">{props.children}</div>
      </Container>
    </dialog>
  )
}
